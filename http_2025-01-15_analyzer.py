...
base_path = 'E:/net_log/zeek/extracted/2025-01-15/http/'  # Jan 15 CTF
...
for log_file in os.listdir(base_path):
    if log_file.startswith('http') and log_file.endswith('.log'):
        ...
        if not df.empty:  # Filter empty DataFrames
            all_dfs.append(df)
...
print("HTTP 4xx/5xx per source IP per hour:", error_counts, sep='\n')
...
# Save outputs
combined_df.to_csv('http_jan15.csv')
with open('http_jan15_output.txt', 'w') as f:
