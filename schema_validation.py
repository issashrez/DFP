...
def validate_all_logs(base_path: str):
    ...
    with ProcessPoolExecutor(max_workers=8) as executor:  # Use all CPU cores
        futures = []
        for log_file in log_files:
            log_type = log_file.parent.name  # Extract from directory name
            futures.append(executor.submit(validate_zeek_schema, log_file, log_type))
        
        results = [f.result() for f in futures]
        return sum(results) / len(results)  # Return pass rate
