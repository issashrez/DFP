...
base_path = 'E:/net_log/zeek/extracted/2025-01-09/dns/'
...
# Feature 3: Average RTT per destination IP per hour (if RTT logged)
avg_rtt = combined_df.groupby(['id.resp_h', pd.Grouper(freq='1h')])['rtt'].mean().infer_objects()
print("Avg RTT per destination IP per hour:", avg_rtt.fillna(0), sep='\n')

# Feature 4: Count of unusual qtype requests per source IP per hour (fixed)
...
# Save outputs
combined_df.to_csv('dns_jan9.csv')
with open('dns_jan9_output.txt', 'w') as f:
