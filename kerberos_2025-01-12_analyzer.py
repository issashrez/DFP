...
base_path = 'E:/net_log/zeek/extracted/2025-01-12/kerberos/'
...
# Feature 3: Count of forwardable ticket requests per source IP per hour
forwardable_conns = combined_df[combined_df['forwardable'] == True]
forwardable_counts = forwardable_conns.groupby(['id.orig_h', pd.Grouper(freq='1h')]).size()
print("Forwardable ticket counts per source IP per hour:", forwardable_counts, sep='\n')

# Save outputs
combined_df.to_csv('kerberos_jan12.csv')
with open('kerberos_jan12_output.txt', 'w') as f:
    ...
    f.write("Forwardable ticket counts per source IP per hour:\n" + forwardable_counts.to_string() + "\n")
